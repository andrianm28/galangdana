...
- **AdminShell.** Today's sidebar has no navigation at all — every admin route is reached by typed URL. This plan adds the six existing routes (Dashboard, Campaigns, Disbursements, Help Articles, Support Tickets) as real nav entries, with status counts rendered in tabular mono. This is a visual-system completion, not a new feature: it surfaces routes that already exist.
