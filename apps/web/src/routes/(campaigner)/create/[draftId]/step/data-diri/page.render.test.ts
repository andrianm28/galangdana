...
describe("data-diri step rendering", () => {
  ...
  test("pre-fills the input from an existing draft answer", () => {
    render(Page, {
      props: {
        data: { draft: { ...DRAFT, answers: { campaignerRole: "Warga setempat" } } },
        params: { draftId: DRAFT.id },
      },
    });
    expect(
      (screen.getByLabelText("Apa peran Anda dalam penggalangan dana ini?") as HTMLInputElement)
        .value,
    ).toBe("Warga setempat");
  });
  ...
  // `data-diri` is only ever the first step of the non_medical track, so
  // previousStep is null here regardless of the DRAFT fixture's track (kept
  // as "medical" above, matching the pre-existing fixture) -- the meaningful
  // assertion is that no save was attempted.
  test("clicking Kembali on a fresh, empty step does not attempt a save", async () => {
    expect(previousStep(DRAFT.track, "data-diri")).toBeNull();
